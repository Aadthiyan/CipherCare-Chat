#!/usr/bin/env python3
"""Test the complete Cipercare API flow"""
...
print("\n" + "="*60)
print("CIPERCARE API END-TO-END TEST")
print("="*60 + "\n")
...
try:
    ...
    if response.status_code == 200:
        ...
        print(f"  Similar records found: {len(result.get('similar_records', []))}")
        if result.get('similar_records'):
            for i, record in enumerate(result.get('similar_records', [])[:3], 1):
                metadata = record.get('metadata', {})
                print(f"    {i}. {metadata.get('condition', 'Unknown')}")
        print(f"  LLM Response preview: {result.get('response', '')[:100]}...\n")
    else:
